...
const connection = mysql.createConnection({
  host: process.env.TIDB_HOST,      // TiDB cluster endpoint
  port: process.env.TIDB_PORT,      // TiDB port (default 4000)
...
app.get('/api/entries', (req, res) => {
  const query = 'SELECT * FROM diary_entries'; // SQL query to fetch all diary entries
  connection.query(query, (err, results) => {
    if (err) {
      console.error('Error fetching entries: ', err);
      return res.status(500).json({ error: err.message });
    }
    res.status(200).json(results); // Return the entries as JSON
  });
});
